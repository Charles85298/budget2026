...
document.getElementById('export-dashboard').onclick=()=>CsvExport.download('financial-freedom-dashboard-bills-'+BillStore.key(displayDate)+'.csv',
  ...
  filteredBills(monthRows()).map(r=>[BillStore.key(displayDate),r.payee,r.due,r.paycheck,r.category,r.method,r.frequency,r.plannedAmount,r.amount,r.actualAmount,r.paidDate,r.funded?'Yes':'No',r.paid?'Yes':'No']));
render();
